...
module.exports = function (context) {
  ...
  if (process.argv.join('|').indexOf('FACEBOOK_ADVERTISER_ID_COLLECTION=') > -1) {
    FACEBOOK_ADVERTISER_ID_COLLECTION = process.argv
      .join('|')
      .match(/FACEBOOK_ADVERTISER_ID_COLLECTION=(.*?)(\||$)/)[1];
  } else {
    FACEBOOK_ADVERTISER_ID_COLLECTION = utilities.getPreferenceValue('FACEBOOK_ADVERTISER_ID_COLLECTION');
  }
  if (
    typeof FACEBOOK_ADVERTISER_ID_COLLECTION == 'string' &&
    FACEBOOK_ADVERTISER_ID_COLLECTION.toLowerCase() === 'false'
  ) {
  ...
  const updatePlistContent = function () {
    ...
    if (FACEBOOK_URL_SCHEME_SUFFIX === '') {
      plistContent = plistContent
        .replace('<key>FacebookUrlSchemeSuffix</key>', '')
        .replace('<string>FACEBOOK_URL_SCHEME_SUFFIX_PLACEHOLDER</string>', '');
    }
    ...
    if (plistContent.indexOf('<key>FacebookAutoLogAppEventsEnabled</key>') === -1) {
      plistContent = plistContent
        .replace('<key>FacebookAutoLogAppEventsEnabled_PLACEHOLDER</key>', '<key>FacebookAutoLogAppEventsEnabled</key>')
        .replace(
          '<string>FACEBOOK_AUTO_LOG_APP_EVENTS_PLACEHOLDER</string>',
          '<' + FACEBOOK_AUTO_LOG_APP_EVENTS + ' />',
        );
    } else {
      plistContent = plistContent
        .replace('<key>FacebookAutoLogAppEventsEnabled_PLACEHOLDER</key>', '')
        .replace('<string>FACEBOOK_AUTO_LOG_APP_EVENTS_PLACEHOLDER</string>', '');
    }
    ...
    if (plistContent.indexOf('<key>FacebookAdvertiserIDCollectionEnabled</key>') === -1) {
      plistContent = plistContent
        ...
        .replace(
          ...
          '<' + FACEBOOK_ADVERTISER_ID_COLLECTION + ' />',
        );
    } else {
      plistContent = plistContent
        .replace('<key>FacebookAdvertiserIDCollectionEnabled_PLACEHOLDER</key>', '')
        .replace('<string>FACEBOOK_ADVERTISER_ID_COLLECTION_PLACEHOLDER</string>', '');
    }
